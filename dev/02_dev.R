...
#' @importFrom bslib page_sidebar sidebar card card_header card_body accordion accordion_panel value_box
      ...
      card(
        ...
        card_header(
          div(
            ...
            h4("Data Preview", class = "mb-0"),
            uiOutput(ns("preview_status"))
          )
        ),
        card_body(
          class = "p-0",
          uiOutput(ns("data_preview_content"))
        )
      )
    )
  )
...
mod_02_data_selection_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # Preview status in main area
    output$preview_status <- renderUI({
      if (is.null(values$raw_data)) {
        span(class = "badge bg-secondary", "No Data")
      } else if (!values$mapping_valid) {
        span(class = "badge bg-warning", "Mapping Required")
      } else {
        span(
          class = "badge bg-success", 
          paste("Ready:", nrow(values$processed_data), "rows")
        )
      }
    })
    
    # Main data preview content
    output$data_preview_content <- renderUI({
      if (is.null(values$raw_data)) {
        div(
          class = "d-flex align-items-center justify-content-center h-100",
          div(
            class = "text-center text-muted",
            icon("upload", class = "fa-3x mb-3"), br(),
            h5("No Data Loaded"),
            p("Upload a CSV or Excel file to begin")
          )
        )
      } else if (!values$mapping_valid) {
        div(
          ...
          div(
            class = "text-center text-muted",
            icon("arrows-alt-h", class = "fa-3x mb-3"), br(),
            h5("Column Mapping Required"),
            p("Map your data columns using the sidebar controls")
          )
        )
      } else {
        DT::DTOutput(ns("data_table"), height = "100%")
      }
    })
